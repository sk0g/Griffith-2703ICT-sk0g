...
function refresh_page($string) {
    $contents = file_get_contents('index_og.html');
    $contents .= "<h3 style='color:red'>" . $string . "</h3>";
    echo $contents;
}
...
if ($name  === "" &&
    $year  === "" &&
    $state === "") {
        refresh_page("At least one field must have a value!");
} elseif ($name  === "" &&
          $state === "" &&
          !is_int($year)) {
              refresh_page("The year has to be an integer");
} else {
